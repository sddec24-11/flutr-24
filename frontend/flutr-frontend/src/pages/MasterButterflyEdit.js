    ...
    const onDrop = useCallback(acceptedFiles => {
      ...
      useEffect(() => {
        const fetchData = async () => {
          ...
          try{
            const response = await fetch(`http://206.81.3.155/api/master/butterflyDetails/${encodeURIComponent(butterflyToEdit)}`,{
              method: 'GET',
              headers: {
      ...
      const handleSubmit = async () => {
        try{
            const response = await fetch("http://206.81.3.155/api/master/editButterfly",{
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': window.sessionStorage.getItem("accessKey"),
